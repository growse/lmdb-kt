...
class LmdbTests {
	...
	fun `given an environment, when fetching the stats with the wrong pagesize, then an assertion error is thrown`() {
		assertThrows<UnableToDetectPageSizeException> {
			Environment(
				Paths.get(
					javaClass.getResource("/databases/little-endian/4KB-page-size/100-random-values")!!.toURI(),
				),
				readOnly = true,
				locking = false,
				byteOrder = ByteOrder.LITTLE_ENDIAN,
...
// 			),
			DatabaseWithStats(
				"/databases/little-endian/not-a-database",
				4096,
				1,
				0L,
				1L,
		...
		fun databasesWithKeysValues(): Stream<Arguments> = /*
			...
			Stream.of(
				...
				arguments(
					...
					16384,
					mapOf(
						"KK123KK".toByteArrayKey() to LengthAndDigest(8, "d2781036b05df83b95dd20f5a497102b"),
					),
				),
				arguments(
					"/databases/little-endian/16KB-page-size/single-large-value",
					16384,
					mapOf(
						"KK123KK".toByteArrayKey() to LengthAndDigest(33000, "79a965574a648d48fc612f28cc49e570"),
					),
				),
				arguments(
					"/databases/little-endian/16KB-page-size/100-random-values",
					...
					Paths.get(
						...
						it.split(",").run {
							this[0].toByteArrayKey() to LengthAndDigest(this[1].trim().toInt(), this[2].trim())
						}
					},
				),
				arguments(
				...
				arguments(
					...
					4096,
					mapOf(
						byteArrayOf(0, 0, 0, 0, 0, 0, 0, 0).toByteArrayKey() to LengthAndDigest(
							224,
							"92ac74587530bc80322bffe124720811",
						),
						byteArrayOf(0, 0, 0, 0, 0, 0, 0, 1).toByteArrayKey() to LengthAndDigest(
							744,
