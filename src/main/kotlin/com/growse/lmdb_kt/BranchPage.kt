...
data class BranchPage(
	...
	constructor(pageHeader: PageHeader, buffer: DbMappedBuffer, number: UInt) : this(
		buffer = buffer,
		number = number,
		pageHeader
	)
	...
	val nodes: List<BranchNode>
		get() {
			...
			return IntRange(1, pageHeader.numKeys())
				...
				.map {
					buffer.readShort().also { logger.trace { "key at $it" } }
				}
				.map {
					BranchNode(buffer, number, it.toUInt())
				}
		}
}
